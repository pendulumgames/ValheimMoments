...
internal static class RelayTests
{
    ...
    internal static void Run()
    {
        ...
        try
        {
            ...
            Check(clientRelay.Offer(client, path, "manual", "x"), "Client can ask host without consulting local Discord settings");
            tick(); tick();
            Check(delivered == 1 && logs.Exists(s => s.StartsWith("Host declined")), "Host disabled/busy refusal does not upload");
            enabled = true;
            for (int i = 0; i < 160; i++) tick();

...
0.18 status supersedes the checkpoints below: opening/ending durations are host configurable; startup sweeps recognized transient files older than 24 hours; compatible participant perspectives can group using host-issued occurrence IDs. A prefix on SendCurrentRandomEvent sends a direct-peer identity plus exact name/time/position snapshot immediately before native SetEvent on the same ordered transport. The client's RPC_SetEvent postfix consumes only an authenticated token whose snapshot matches exactly, never a time window. Missing/mismatched identity remains personal. The host creates a new GUID for a new RandomEvent instance. Same-instance token changes abandon an existing attempt. Dedicated hosts install identity coordination before the graphics-only return. Tests exercise stable IDs, sender authentication, exact pairing, mismatch and token consumption with real Harmony and behavioral stand-ins; live transport ordering/grouping remains an acceptance item.

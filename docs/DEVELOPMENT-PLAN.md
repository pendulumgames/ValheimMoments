...
**The director is not active in gameplay yet.** The owner metadata hook is implemented, but Plugin and ClipRelay do not yet pass offers into the selector or transfer its selected perspectives to the grouped uploader. Next: host-owned director configuration, session-scoped clip metadata propagation, bounded disk-backed relay/admission and completion/omission integration. The selector trusts host-supplied authenticated peer identities; a caller must never substitute client-provided recorder names or webhook settings. Deferred live tests must include two simultaneous same-name bosses, two/three participants, missing/late offers, dedicated ownership and host disconnect.

...
The accepted feature categories are now implemented across 0.12-0.18: simplified host-owned Discord settings and boss selection; aspect-aware capture presets and clamps; local saving, notifications and death quotas/flavor; per-character/world discoveries and configurable special enemies; bounded multiplayer perspective selection; segmented raids and survival-validated close calls; and personal gallery/Keep/recovery. Earlier progress sections below are historical snapshots.

0.18 adds player-owned F8 gallery and F7 Keep bindings, lazy retained thumbnails, actual local animation opening, authenticated delivery receipt links when available, and bounded history/recovery. Permanent Saved originals never participate in transient eviction. Encoding/upload leases prevent pin/cleanup races; successful unpinned footage gets a 30-second grace. Retry is deliberately manual, three attempts with 30/60/120-second backoff, restricted to the original in-memory session and role. Unknown delivery requires duplicate-post confirmation; no auto-resend. Death retries reserve their original acknowledgement snapshot. Invalid URL/auth/oversize outcomes require correction before retry. Old-session records cannot silently use a new destination.

Host settings now control close-call source/follow-up/slow/total playback and raid opening/ending durations. Independent ranges maintain a positive fast segment; pool fitting accounts for selected frames and the aggregate raid composition budget. Raid grouping uses a host-issued GUID paired with the exact native SetEvent snapshot over the authenticated ordered peer connection. Missing/mismatched tokens stay personal. Local area participation and survival cancellation remain mandatory; administrative endings still say Raid ended.

Startup sweeping covers only recognized old RelayTemp/RaidTemp files; gallery handles its owned recovery files. Unknown files, redirected directories and permanent Saved originals are preserved. Existing historical Clips files are intentionally not imported/deleted. Thumbnail files are bounded by retained history. Gallery input uses inspected Player.TakeInput/GameCamera.UpdateMouseCapture hooks; the player is not paused in multiplayer.

Release choices: the director uses the accepted select-fewer-or-omit fallback, with explicit omission and recovery, rather than adding repeated expensive re-encoding. Client transport retains its tested conservative 10 MiB cap; configured Discord budgets are guards, not automatic boost-tier discovery. Optional separate raid-start posts and larger transport/re-encoding remain future enhancements. Local animation playback opens the registered Windows viewer; deleted media has only a retained thumbnail/available message link. These limits are disclosed in player documentation.

Remaining acceptance requires live gameplay, not additional unattended implementation: gallery/cursor/input visuals, chosen key conflicts, two/three-player and dedicated raid grouping, customized timeline playback, HUD/audio appearance, storage on real installations, and sustained performance. Representative gameplay size measurements cannot be produced while the user is playing without interrupting them; publish no invented typical-size table. Use docs/RELAY-TEST.md for the consolidated final pass. No install or Thunderstore submission is authorized by packaging alone.
...
Segment assembly checkpoint: the encoder helper can combine two bounded animated WebP inputs via sequential decode/re-encode. It validates compressed size, canvas/frame rectangles, matching dimensions, frame count, duration and raw-equivalent budget before decoding. Full-decode tests verify exact irregular timing/colors at the segment boundary, overwrite protection, malformed-input rejection and normal-failure partial cleanup. Existing close-call and ordinary encoder checks remain green. Parent-owned intermediate retention, kill/crash cleanup, opening/ending capture orchestration and gameplay registration remain pending. No new package or installation; 0.16.0 archives are unchanged.
...
Death cancels pending close-call footage before normal death quota/admission. Encoding requires both alive follow-up completion and the readback submission watermark. Pause, host-policy loss, character/session change and capture reconfiguration abandon collection. Busy attempts are skipped with recovery/cooldown still required. Close-call posts remain personal, even with the multiplayer director enabled. They use the host's default webhook and authenticated recorder identity. Matching 0.16.0 host and clients are required.

Release checks passed 2,411 assertions (including 408 close-call policy/collector checks and five damage-hook checks), production/helper builds, exact 10,000 ms synthetic WebP timing and full decode, 94 documented config entries, 19-entry package validation, native binary provenance and the installed Configuration Manager attribute contract. Live gameplay/visual/performance acceptance remains deferred in docs/RELAY-TEST.md. No game installation or Thunderstore submission. Raid survival clips, configurable timeline durations, gallery/Keep/recovery and further director refinements remain pending.

## Earlier close-call implementation checkpoints (included in 0.16.0)

Follow-up checkpoint: CaptureBuffer now supports a sampled close-call collection using its existing fixed pixel pool. Admission checks the selected frame budget and available history; it rejects insufficient capacity without allocation. It preserves one second of history, samples the twenty-second follow-up, waits for the GPU submission watermark and exposes remapped playback timestamps to the existing encoder. Cancellation immediately permits an ordinary death capture. Core tests now include 408 close-call assertions plus the existing 1,244. The actual encoder produced a fully decodable synthetic close-call WebP with exactly 10,000 ms of RIFF animation duration; existing encode/decode/cancellation checks still pass.

LocalDamageDetector compiles against the installed Character.ApplyDamage/GetHealth/GetMaxHealth API and has five real-Harmony behavioral checks for local actual damage, unchanged health, remote/nonowner exclusions and observer exception isolation. Existing event/relay tests pass. The detector is deliberately not installed by Plugin yet: settings, policy/session observation, survival-gated completion, death priority and message routing must be wired together before enabling this event. Production includes the new core classes, but this checkpoint is not a playable close-call release. Raid API/outcome investigation remains pending. No package rebuild or installation.

Added isolated Core CloseCall and CloseCallTimeline components, with 279 automated assertions alongside the existing 1,244 core assertions. Damage must cross from above to at/below 5% with an unchanged positive maximum health; polling, starting low and denominator changes cannot trigger. Completion requires an alive observation at/after the 20-second follow-up; death at that boundary cancels. Attempts remain disarmed until both the 120-second cooldown and 10 seconds continuously observed at/above 20% recovery pass. Damage below recovery interrupts the hold even between update observations. Cancelled/busy attempts cannot bypass rearming; character/world transitions must explicitly reset the controller.

The timeline maps [-1, 0] source seconds to [0, 3000] playback milliseconds and [0, 20] to [3000, 10000]. At 15 FPS the fast portion needs only 105 scheduled source samples. This is a coordinate/sampling policy, not yet a frame collector or encoder integration. Tests verify exact endpoints, monotonic mapping, invalid values, cooldown/recovery, periodic oscillation and cancellation. No gameplay hooks, settings, raid detection, new package or installation in this checkpoint. Released 0.15.0 artifacts remain unchanged. Next: inspect game damage/raid APIs, connect a bounded segmented collector with death priority and GPU watermark handling, then validate actual decoded animation timing before release.

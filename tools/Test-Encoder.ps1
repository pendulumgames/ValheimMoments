...
& $compiler /nologo /warnaserror+ /optimize+ /target:exe "/out:$exe" (Join-Path $workspace 'src/ValheimMoments.Core/CaptureBuffer.cs') (Join-Path $workspace 'src/ValheimMoments/EncoderClient.cs') (Join-Path $workspace 'tests/EncoderSmokeTests.cs')
